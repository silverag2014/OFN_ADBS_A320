...
unsigned char Feature_report_buffer[10];
unsigned char Feature_IN_Report[10];
unsigned char Feature_OUT_Report[10];

unsigned char report_buffer[3];
extern unsigned char erase_flash;
extern data unsigned char IN_PACKET[10];
extern data unsigned char OUT_PACKET[5];
extern data unsigned char grab_image;
extern xdata unsigned char image_buffer[PIXEL_NUMBER + 2];
extern data unsigned char received;

extern signed char x_raw, y_raw;
extern unsigned char k_raw;
extern unsigned char communication_mode;
...
void Handle_Control (void)
{
   ...
   if (EP_STATUS[0] == EP_RX)          // See if endpoint should transmit
   {
      ...
      if (ControlReg & rbOPRDY)        // Verify packet was received
      {
         ...
         {
		 	if((SETUP.wValue.c[MSB] == HID_FEATURE)
			&& (SETUP.wValue.c[LSB] == 1)			// report ID     
			&& (SETUP.wLength.i == 4))				// length
			{
				if(Feature_report_buffer[0] == 1)
				{
					switch(Feature_report_buffer[1])
					{
						case 0x30:
							if(communication_mode == SPI_MODE)
							{
								spi_write_sensor_int(Feature_report_buffer[2], Feature_report_buffer[3]);
							}
							else if(communication_mode == I2C_MODE)
							{
								while(i2c_status != I2C_FREE);
								i2c_ByteWrite(I2C_ADDR, Feature_report_buffer[2], &Feature_report_buffer[3], 1);
								while(i2c_status != I2C_FREE);
							}
							break;
						case 0x31:
							if(communication_mode == SPI_MODE)
							{
								Feature_report_buffer[3] = spi_read_sensor_int(Feature_report_buffer[2]);
							}
							else if(communication_mode == I2C_MODE)
							{
								while(i2c_status != I2C_FREE);
								i2c_ByteRead(I2C_ADDR, Feature_report_buffer[2], report_buffer, 1);
								while(i2c_status != I2C_FREE);
								Feature_report_buffer[3] = report_buffer[0];
							}
							break;
						case 0x33:
							image_buffer[1] = 0;
							grab_image = 1;     // Grab Image;
							break;
						case 0x34:
							Feature_report_buffer[2] = image_buffer[1];
							break;
						case 0x40:
							break;
						case 0x41:
						...
						case 0x52:
							break;
						case 0x61:
							switch(Feature_report_buffer[2])
							{
								case 0x01:
									Feature_report_buffer[3] = x_raw;
									x_raw = 0;
									break;
								case 0x02:
									Feature_report_buffer[3] = y_raw;
									y_raw = 0;
									break;
								case 0x03:
									Feature_report_buffer[3] = k_raw;
									k_raw = 0;
									break;
								default:
									break;
							...
							break;
						case 0x71:
							//Feature_report_buffer[2] = (unsigned char) FIRMWARE_VERSION;
							//Feature_report_buffer[3] = (unsigned char) ((FIRMWARE_VERSION * 10) - ((unsigned char) FIRMWARE_VERSION * 10));
							Feature_report_buffer[2] = FIRMWARE_VERSION_MAJOR;
							Feature_report_buffer[3] = FIRMWARE_VERSION_MINOR;
							break;
						case 0x91:
							image_buffer[0] = Feature_report_buffer[1];
							image_buffer[1] = 1;
							switch(Feature_report_buffer[2])
							{
								case 0x01: // customer type
									sprintf(&image_buffer[2], "%s", CUSTOMER);
									break;
								case 0x02: // nature
									sprintf(&image_buffer[2], "%s", SAMPLE_TYPE);
									break;
								default:
									image_buffer[1] = 0;
									break;
			...
			else if((SETUP.wValue.c[MSB] == HID_OUTPUT_REPORT)
			&& (SETUP.wValue.c[LSB] == 0)			// report ID     
			&& (SETUP.wIndex.i == 0)
			&& (SETUP.wLength.i == 3))				// length
			{
				received = 1;
