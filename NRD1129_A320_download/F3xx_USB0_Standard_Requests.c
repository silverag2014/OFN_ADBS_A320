...
// Variables
//-----------------------------------------------------------------------------
// These are created in F3xx_USB0_Descriptor.h
extern code device_descriptor DEVICEDESC;   
...
extern unsigned char Feature_IN_Report[10];
extern unsigned char Feature_OUT_Report[10];
...
void Get_Report (void)
{
    ...
    bit handled = 0;  
	bit nack = 0;  
	if(SETUP.wValue.c[ MSB ] == HID_FEATURE)	// Get_Report(Feature)
	{
		temp1 = SETUP.wValue.c[LSB];
		temp2 = SETUP.wLength.i ;

		if((SETUP.wValue.c[LSB] == 1)			// report ID     
		&& (SETUP.wLength.i == 4))				// length
		{
			DATASIZE = 4;
			DATAPTR  = Feature_report_buffer;
			handled  = 1;
		}
		else 
		if((SETUP.wValue.c[LSB] == 2)			// report ID     
	 	&& (SETUP.wLength.i == (PIXEL_NUMBER + 2)))
		{
			DATASIZE = SETUP.wLength.i;
			DATAPTR  = image_buffer;
			handled  = 1;
		}
	}
	else  
	if((SETUP.wValue.c[ MSB ] == 0)      // Get_Report( Input )        
	&& (SETUP.wLength.i       == 9))
	{            
		DATASIZE = 9;
		DATAPTR  = IN_PACKET;
...
{
	if (   (SETUP.wValue.c[MSB] == HID_FEATURE )        // Set_Report( Feature )     
	    && (SETUP.wValue.c[LSB] == 1)                   // default report ID     
		&& (SETUP.wIndex.i      == 0)                   // Interface 0    
		&& (SETUP.wLength.i     == 4 ) ) // report size    
	{        
		DATASIZE = 4;
		DATAPTR  = Feature_report_buffer;        // Set serviced SETUP Packet        
		POLL_WRITE_BYTE (E0CSR, rbSOPRDY);        
